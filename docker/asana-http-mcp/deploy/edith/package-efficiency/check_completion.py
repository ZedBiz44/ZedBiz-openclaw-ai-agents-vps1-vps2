...
# Project-specific user exception; general file inspection remains the default.
PHOTOSHOP_POLICY = 'jack-2026-09-17-legacy-photoshop'
PHOTOSHOP_TASKS = {'1218559788929226','1218559788686767','1218559621792077','1218559536949282','1218559788863817','1218559621676002','1218559788958539','1218559452201256','1218574348046598','1218559536996899'}
def photoshop_exception(row, plan, source):
    from pathlib import PurePosixPath
    if plan.get('project_gid') != '1218559074752632' or plan.get('task_gid') not in PHOTOSHOP_TASKS:
        return False
    if row.get('preservation_exception') != PHOTOSHOP_POLICY or row.get('inspected') is not False:
        return False
    name = source.get('name', '')
    target = PurePosixPath(row.get('planned_path', ''))
    return (source.get('id') == row.get('source_id')
            and source.get('mimeType') != 'application/vnd.google-apps.folder'
            and name.lower().endswith(('.psd', '.psb'))
            and target.name == name and not target.is_absolute() and '..' not in target.parts
            and any(p == 'Photoshop-Source-Files' or p.endswith('-Photoshop-Source-Files') for p in target.parts[:-1])
            and row.get('inspection_method') == 'metadata-only: user-approved Photoshop preservation'
            and bool(row.get('inspection_evidence')) and bool(source.get('md5Checksum')) and source.get('size') is not None)



PACKAGE_POLICY='jack-2026-09-17-retained-package'
DOCUMENT_EXTENSIONS={'.pdf','.doc','.docx','.odt','.rtf','.txt','.md','.csv','.xls','.xlsx','.ods','.ppt','.pptx','.odp','.epub'}
ARCHIVE_EXTENSIONS={'.zip','.7z','.rar','.tar','.gz','.tgz','.bz2','.xz'}
def package_exception(row, plan, source, before):
    from pathlib import PurePosixPath
    if plan.get('project_gid') != '1218559074752632' or plan.get('task_gid') not in PHOTOSHOP_TASKS:
        return False
    if row.get('preservation_exception') != PACKAGE_POLICY or row.get('inspected') is not False:
        return False
    if row.get('inspection_method') != 'metadata-only: user-approved package preservation' or not row.get('inspection_evidence'):
        return False
    target=PurePosixPath(row.get('planned_path',''))
    if target.is_absolute() or '..' in target.parts or target.name != source.get('name') or not any(p=='Retained-Packages' or p.endswith('-Retained-Packages') for p in target.parts[:-1]):
        return False
    if source.get('id') != row.get('source_id') or not source.get('md5Checksum') or source.get('size') is None:
        return False
    originals={x['id']:x for x in before.get('items',[])}
    package=next((x for x in plan.get('preserved_packages',[]) if x.get('root_id')==row.get('package_root_id')),None)
    if not package or not package.get('listing_evidence') or not package.get('purpose') or package.get('kind') not in {'saved-website','software','extracted-bundle','archive'}:
        return False
    root=originals.get(package['root_id'],{})
    if package['kind']=='archive':
        return (source['id']==package['root_id'] and PurePosixPath(source['name']).suffix.lower() in ARCHIVE_EXTENSIONS
                and package.get('document_triage') in {'listed','deferred-encrypted','deferred-too-large','deferred-unsupported'}
                and isinstance(package.get('useful_documents'),list)
                and (package['document_triage']=='listed' or bool(package.get('defer_reason'))))
    if root.get('mimeType')!='application/vnd.google-apps.folder' or package.get('document_triage')!='listed':
        return False
    if source.get('mimeType','').startswith('application/vnd.google-apps.') or PurePosixPath(source['name']).suffix.lower() in DOCUMENT_EXTENSIONS:
        return False
    # Establish package membership by IDs, not filename guesses.
    cur=source;seen=set()
    while cur.get('id')!=root.get('id'):
        if cur.get('id') in seen or len(cur.get('parents',[]))!=1:return False
        seen.add(cur.get('id'));cur=originals.get(cur['parents'][0],{})
        if not cur:return False
    try:
        relative=PurePosixPath(source['path']).relative_to(PurePosixPath(root['path']))
    except (KeyError,ValueError):return False
    package_target=PurePosixPath(package.get('planned_root',''))
    if str(target)!=str(package_target/relative):return False
    # Every readable document in the extracted package must receive normal review.
    docs=[]
    for item in originals.values():
        if item.get('mimeType')=='application/vnd.google-apps.folder':continue
        try:PurePosixPath(item['path']).relative_to(PurePosixPath(root['path']))
        except (KeyError,ValueError):continue
        if PurePosixPath(item.get('name','')).suffix.lower() in DOCUMENT_EXTENSIONS or item.get('mimeType','').startswith('application/vnd.google-apps.'):
            docs.append(item['id'])
    if set(package.get('useful_documents',[]))!=set(docs):return False
    rows={x.get('source_id'):x for x in plan.get('file_plan',[])}
    return all(d in rows and not rows[d].get('preservation_exception') for d in docs)

def preservation_exception(row,plan,source,before):
    return photoshop_exception(row,plan,source) or package_exception(row,plan,source,before)
...
def check_file_plan(plan, before):
    ...
    source_by_id = {x['id']: x for x in before['items']}
    for row in rows:
        if row.get('preservation_exception') and not preservation_exception(row, plan, source_by_id.get(row.get('source_id'), {}), before):
            issues.append({'type': 'invalid_preservation_exception', 'id': row.get('source_id')})
        ...
        if inspection_unavailable(row) and not preservation_exception(row, plan, source_by_id.get(row.get('source_id'), {}), before):
...
def check_completion(data):
    ...
    for fid, source in originals.items():
        ...
        exemption = preservation_exception(row, plan, source, before) and any(p.get('source_id') == fid and preservation_exception(p, plan, source, before) for p in plan.get('file_plan', []))
        if (row.get('inspected') is not True and not exemption) or any(not isinstance(row.get(k), str) or not row[k].strip() for k in required):
            issues.append({'type': 'incomplete_file_evidence', 'id': fid})
        if inspection_unavailable(row) and not exemption:
