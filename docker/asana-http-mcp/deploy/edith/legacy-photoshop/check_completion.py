...
def check_file_plan(plan, before):
    ...
    for row in rows:
        if row.get('preservation_exception') and not photoshop_exception(row, plan, source_by_id.get(row.get('source_id'), {})):
        ...
        if inspection_unavailable(row) and not photoshop_exception(row, plan, source_by_id.get(row.get('source_id'), {})):
...
def check_completion(data):
    ...
    for fid, source in originals.items():
        ...
        exemption = photoshop_exception(row, plan, source) and any(p.get('source_id') == fid and photoshop_exception(p, plan, source) for p in plan.get('file_plan', []))
