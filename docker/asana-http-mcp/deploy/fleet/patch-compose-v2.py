#!/usr/bin/env python3
"""Configure one VPS1 agent with the Standard or Advanced Asana HTTP MCP."""
...
IMAGES = {
    "standard": "zedbiz/asana-http-mcp:2.0.0-standard",
    "advanced": "zedbiz/asana-http-mcp:2.0.0-advanced",
}
...
def main() -> None:
    ...
    parser.add_argument("--toolset", choices=sorted(IMAGES), required=True)
    ...
    old_team_sidecar = f"{args.agent}-asana-team-mcp"
    ...
    if args.toolset == "advanced":
        service["command"] = ["node", "dist/advanced-server.js"]
    ...
    print(f"Configured {args.agent} with {args.toolset} Asana toolset")
