...
description: Capture and deliver actual visual screenshots of public webpages or pages available through an authorized browser session.
---
...
Create an artifact directory inside the active workspace and use Chromium. Set a clean, unique user-data directory to avoid profile-lock errors.

```bash
workspace_dir="${OPENCLAW_WORKSPACE_DIR:-$PWD}"
artifact_dir="$workspace_dir/artifacts/site-screenshots/$(date +%Y%m%d-%H%M%S)"
