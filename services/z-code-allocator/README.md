...
Date: 2026-09-14 | Author: Cody | Status: Production
...
The allocator website is the controlled editing point for Topic Name, Name-Key, and Knowledge Family. Notion is the readable mirror; do not use direct Notion edits as a reverse-sync route.
...
- Authentication failures pause the mirror queue instead of repeatedly retrying every item. Other failures use capped exponential backoff.
...
The normal write-capable dashboard is served by `z-code-admin`. It provides Records, Topics, Retired & Aliases, and Mirror Health views. Controlled changes and resyncs create both an audit event and a Notion outbox event.

- Public path: `https://zcode-admin.zbiz.ca/`
