...
- **Endpoint-reference backup:** `/opt/openclaw/agents/wilma/backups/20260802-1334MDT-1password-endpoint-refs`
...
- Both route URLs now use `${WP_MCP_ENDPOINT_ZEDBIZ_COM}` and `${WP_MCP_ENDPOINT_TOPZED_COM}`. The actual endpoint addresses are resolved from the `agent-wilma` 1Password vault when Wilma starts.
...
- The two endpoint items in the `agent-wilma` 1Password vault were corrected by a human vault editor to `/v2/http`. Wilma's service account then retrieved and verified both corrected values.
...
- After replacing the literal route URLs with 1Password-backed endpoint references, Wilma was cleanly restarted and both native probes passed again with six tools and zero diagnostics.
